...
const CompanyProfile: React.FC = ():JSX.Element => {
  ...
  return (
    ...
    
    {
      data ? 
      <div>
        <Metric title={'Market Cap'}  description={"Total value of all a company's shares of stock"} value={data.marketCapTTM} Icon ={PaidIcon}/>
        <Metric title={'Current Ratio'}  description={"Measures the companies ability to pay short term debt obligations"} value={data.currentRatioTTM} Icon ={PriceChangeIcon}/>
        ...
        <Metric title={'Divdend Yield TTM'}  description={"Shows how much a company pays each year relative to stock price"} value={data.dividendYieldTTM} Icon ={DiamondIcon}/>
        ...
        <Metric title={'Graham Number'}  description={"Estimates the maximum fair price for a stock based on its earnings and book value"} value={data.grahamNumberTTM} Icon ={PaidIcon}/>
        <Metric title={'PE Ratio'}  description={"This is the upperbouind of the price range that a defensive investor should pay for a stock"} value={data.peRatioTTM} Icon ={ShowChartIcon}/>
      </div> 
